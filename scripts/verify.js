/*
 * End-to-end verification of the deployed surface. Serves nothing itself:
 * point BASE_URL at a running local server (default http://127.0.0.1:8317).
 *
 *   python3 -m http.server 8317 &   # from the repo root
 *   node scripts/verify.js
 *
 * Needs the playwright package resolvable (local node_modules or NODE_PATH).
...
(async () => {
  ...
  await pa.goto(BASE + "/about.html", { waitUntil: "networkidle" });
  ...
  check("physical link degrades to coming-soon (empty config URL)",
    (await pa.locator("[data-physical-link]").count()) === 0 && (await pa.textContent("main")).includes("coming soon"));
  check("deck version shown discreetly", (await pa.textContent(".credit")).includes("Deck v"));
  const pad = await ctxDark.newPage();
  await pad.goto(BASE + "/about.html", { waitUntil: "networkidle" });
