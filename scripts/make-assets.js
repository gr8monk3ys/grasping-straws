...
// Mirrors the tokens in src/styles/global.css. The og image is a product
// shot of the site, so the ground is the dark room — that is the version
// the card is meant to be seen against, and it survives a social feed's
// own background far better than the pale one.
const BG = "#0d1514";
const CARD = "#fbf7ee";
const INK = "#191713";
const MUTED = "#6f6653";
...
const ogHtml = `<!doctype html><style>${baseCss}
  /* 11/19 — the tarot ratio the physical deck is cut to. */
  .card { width: 300px; height: 518px; background: ${CARD};
          border-radius: 18px; position: relative;
          box-shadow: 0 3px 8px rgb(0 0 0 / 0.5), 0 34px 80px -26px rgb(0 0 0 / 0.85);
          display: grid; place-items: center; }
  .card::before { content: ""; position: absolute; inset: 18px;
                  border: 1.5px solid ${LINE}; border-radius: 8px; }
...
const iconHtml = (size) => `<!doctype html><style>${baseCss}
  svg { width: ${Math.round(size * 0.58)}px; height: ${Math.round(size * 0.58)}px; color: ${CARD}; }
</style><body>${svg}</body>`;
